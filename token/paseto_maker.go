...
func (maker *PasetoMaker) CreateToken(userId uuid.UUID, duration time.Duration, purpose string) (string, *Payload, error) {
	payload, err := NewPayLoad(userId, duration, purpose)
	if err != nil {
		return "", nil, err
	}
